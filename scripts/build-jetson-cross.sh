...
docker "${DOCKER_ARGS[@]}" /bin/bash -lc \
    "export BUILD_DIR_NAME='${BUILD_DIR_NAME}'; export ENABLE_JETSON_INFERENCE='${ENABLE_JETSON_INFERENCE}'; export ENABLE_TENSORRT_MASK='${ENABLE_TENSORRT_MASK}'; export JETSON_SYSROOT_IN_CONTAINER='${CONTAINER_SYSROOT}'; export JETSON_INFERENCE_ROOT='${CONTAINER_JETSON_INFERENCE_ROOT}'; export HOST_UID='${HOST_UID}'; export HOST_GID='${HOST_GID}'; ${CONTAINER_COMMAND}"
