...
- `--secondary-camera-pipeline <pipeline>`: GStreamer pipeline for the secondary camera source. The pipeline must end in an `appsink` that produces BGR frames. For the external `uxplay` TCP setup, use `tcpclientsrc host=127.0.0.1 port=5000 ! rawvideoparse format=bgr width=1280 height=720 framerate=30/1 ! video/x-raw,format=BGR,width=1280,height=720,framerate=30/1 ! appsink drop=true max-buffers=1 sync=false`.
...
```json
{
  ...
  "secondaryCamera": {
    "pipeline": "tcpclientsrc host=127.0.0.1 port=5000 ! rawvideoparse format=bgr width=1280 height=720 framerate=30/1 ! video/x-raw,format=BGR,width=1280,height=720,framerate=30/1 ! appsink drop=true max-buffers=1 sync=false"
  },
  "pause": {
    "enabled": false,
    "source": "image",
