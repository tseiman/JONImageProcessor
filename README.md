...
- `--pause-source <image|camera>`: pause media source. `image` uses `--pause-image`; `camera` uses the configured secondary camera pipeline. Default: `image`.
- `--secondary-camera-pipeline <pipeline>`: GStreamer pipeline for the secondary camera source. The pipeline must end in an `appsink` that produces BGR, RGB, BGRx, or BGRA frames. For the external `uxplay` TCP setup, run `/usr/local/bin/uxplay -nc -n "Linux-OpenCV" -fps 20 -vp "h264parse ! nvv4l2decoder ! nvvidconv ! video/x-raw(memory:NVMM),format=NV12 ! nvvidconv ! video/x-raw,format=BGRx" -vs "gdppay ! queue leaky=downstream max-size-buffers=1 ! tcpserversink host=127.0.0.1 port=5000 sync=false" -as 0 -reset 0 -nohold` and configure JONImageProcessor with `tcpclientsrc host=127.0.0.1 port=5000 ! gdpdepay ! queue leaky=downstream max-size-buffers=1 ! appsink name=airplay_sink drop=true max-buffers=1 sync=false`. Do not add `videoscale add-borders=true` or force `width=1280,height=720` in the UxPlay pipeline when `pause.preserveAspectRatio=true`; JONImageProcessor needs the real incoming frame size so it can create the black bars itself.
...
```json
{
  ...
  "secondaryCamera": {
    "pipeline": "tcpclientsrc host=127.0.0.1 port=5000 ! gdpdepay ! queue leaky=downstream max-size-buffers=1 ! appsink name=airplay_sink drop=true max-buffers=1 sync=false"
  },
  "pause": {
    "enabled": false,
    "source": "image",
...
- `secondaryCamera.pipeline`: read-only GStreamer pipeline used when `pause.source=camera`.
...
```bash
echo '{"cmd":"get","key":"secondaryCamera.pipeline"}' | socat - UNIX-CONNECT:/tmp/jonimageprocessor.sock
```
