...
- `--pause-source <image|camera>`: pause media source. `image` uses `--pause-image`; `camera` uses the secondary camera source. Default: `image`.
- `--secondary-camera-device <path>`: secondary camera device used when `--pause-source camera` is selected and no secondary camera pipeline is configured. Default: `/dev/video10`.
- `--secondary-camera-pipeline <pipeline>`: GStreamer pipeline for the secondary camera source. When set, it overrides `--secondary-camera-device`. The pipeline must end in an `appsink` that produces BGR frames.
...
- `secondaryCamera.device`: secondary camera device used when `pause.source=camera` and no pipeline is configured, for example `/dev/video10`.
- `secondaryCamera.pipeline`: read-only GStreamer pipeline used when `pause.source=camera`; overrides `secondaryCamera.device` when set.
