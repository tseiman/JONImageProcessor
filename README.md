...
- `--secondary-camera-pipeline <pipeline>`: GStreamer pipeline for the secondary camera source. The pipeline must end in an `appsink` that produces BGR, RGB, BGRx, or BGRA frames. For the external `uxplay` TCP setup, run `/usr/local/bin/uxplay -nc -n "Linux-OpenCV" -fps 20 -vp "h264parse ! nvv4l2decoder ! nvvidconv ! video/x-raw(memory:NVMM),format=NV12,width=1280,height=720 ! nvvidconv ! video/x-raw,format=BGRx,width=1280,height=720" -vs "gdppay ! queue leaky=downstream max-size-buffers=1 ! tcpserversink host=127.0.0.1 port=5000 sync=false" -as 0 -reset 0 -nohold` and configure JONImageProcessor with `tcpclientsrc host=127.0.0.1 port=5000 ! gdpdepay ! queue leaky=downstream max-size-buffers=1 ! appsink name=airplay_sink drop=true max-buffers=1 sync=false`.
