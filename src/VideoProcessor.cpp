...
struct PauseCameraSource {
    ...
    static int h264EndMarkerNalTypeFromRtpPayload(const std::uint8_t* payload, std::size_t size)
    {
        if (!payload || size == 0) {
            return 0;
        }
        ...
        if (nalType == 24) {
            ...
            while (offset + 2 < size) {
                ...
                const int stapNalType = payload[offset] & 0x1f;
                if (stapNalType == 10 || stapNalType == 11) {
                    return stapNalType;
                }
                offset += nalSize;
            }
        } else if ((nalType == 28 || nalType == 29) && size >= 2) {
            const int fragmentNalType = payload[1] & 0x1f;
            if (fragmentNalType == 10 || fragmentNalType == 11) {
                return fragmentNalType;
            }
        }
    ...
    static int h264EndMarkerNalTypeFromRtpBuffer(GstBuffer* buffer)
    {
        if (!buffer) {
            return 0;
        }
        ...
        const int nalType = h264EndMarkerNalTypeFromRtpPayload(
            static_cast<const std::uint8_t*>(gst_rtp_buffer_get_payload(&rtpBuffer)),
            gst_rtp_buffer_get_payload_len(&rtpBuffer));
    ...
    void ensureWorkerStarted()
    {
        ...
        workerState->worker = std::thread([workerState]() {
            ...
            auto buildDecoderPipeline = [&](const std::string& targetSig) -> DecoderStartResult {
                clearDecoderPipeline();
                ...
                if (!decoderPipeline) {
                    ...
                    return DecoderStartResult::Failed;
                }
                if (parseError) {
                ...
                if (cachedKeyframe && cachedKeyframeCaps && cachedKeyframeSig == targetSig) {
                    ...
                    if (ret == GST_FLOW_OK) {
                        ...
                        return DecoderStartResult::StartedWithIDR;
                    } else {
                ...
                return DecoderStartResult::StartedWithoutIDR;
            };
#else
            int lastOpenFailurePort = 0;
#endif
            ...
            while (!workerState->stop) {
                ...
                if (!rtpPipeline || !rtpSink || localRtpPort != wantedRtpPort) {
                    ...
                    GstPad* depaySinkPad = depayElem ? gst_element_get_static_pad(depayElem, "sink") : nullptr;
                    if (depaySinkPad) {
                        gst_pad_add_probe(depaySinkPad, GST_PAD_PROBE_TYPE_BUFFER,
                            [](GstPad*, GstPadProbeInfo* info, gpointer userData) -> GstPadProbeReturn {
                                auto* currentState = static_cast<State*>(userData);
                                GstBuffer* probeBuffer = GST_PAD_PROBE_INFO_BUFFER(info);
                                const int nalType = h264EndMarkerNalTypeFromRtpBuffer(probeBuffer);
                                if (nalType == 10 || nalType == 11) {
                                    std::lock_guard<std::mutex> lock(currentState->mutex);
                ...
                if (!sample) {
                    const auto now = std::chrono::steady_clock::now();
                    if (now - lastSampleAt > noSampleLogTimeout) {
                        std::lock_guard<std::mutex> lock(workerState->mutex);
                        if (workerState->hasFrame) {
                            if (lastNoSampleLog.time_since_epoch().count() == 0
                                || now - lastNoSampleLog >= std::chrono::seconds(30)) {
                                LOG_INFO("AirPlay RTP: no video samples for "
                                    << std::chrono::duration_cast<std::chrono::seconds>(noSampleLogTimeout).count()
                                    << "s, keeping last frame");
                                lastNoSampleLog = now;
                            }
                ...
                if (isKeyframe) {
                    ...
                    if (!decoderPipeline && !cachedKeyframeSig.empty()) {
                        ...
                        if (buildDecoderPipeline(cachedKeyframeSig) == DecoderStartResult::StartedWithIDR) {
                    ...
                    if (waitingForMatchingKeyframe && bufferSig == targetCapsSig) {
                        ...
                        if (buildDecoderPipeline(targetCapsSig) == DecoderStartResult::StartedWithIDR) {
                            waitingForMatchingKeyframe = false;
                            waitingForFirstFrame = true;
                            fallbackStartedAt = std::chrono::steady_clock::now();
                            clearPendingFallback();
                ...
                if (waitingForMatchingKeyframe) {
                    ...
                    if (now - waitingForKeyframeSince >= idrWaitTimeout && pendingFallbackBuffer && pendingFallbackCaps) {
                        ...
                        const DecoderStartResult startResult = buildDecoderPipeline(targetCapsSig);
                        if (startResult != DecoderStartResult::Failed) {
                            waitingForMatchingKeyframe = false;
                            waitingForFirstFrame = true;
                            fallbackStartedAt = now;
                            if (startResult == DecoderStartResult::StartedWithoutIDR && decoderSrc) {
                            ...
                            } else if (startResult == DecoderStartResult::StartedWithIDR) {
                                LOG_INFO("AirPlay decoder pipeline started with cached IDR, skipping fallback buffer (" << targetCapsSig << ")");
                            }
                            clearPendingFallback();
                        }
                        continue;
